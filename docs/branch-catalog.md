# Unsafe Branch and Pull Request Catalog

`main` contains the secure examples. Every `demo/*` branch removes one
protection so one expected check fails.
...
| `CI-01` | [`demo/05-unpinned-action`](https://github.com/adnanbn/5p_security_demos/tree/demo/05-unpinned-action) | [PR #5](https://github.com/adnanbn/5p_security_demos/pull/5) | Audit GitHub Actions | Workflow dependencies run code inside CI |
...
The draft pull requests are intentionally failing examples. They must not be
merged.

## Demonstration Pattern

1. Open the draft pull request and predict which check should fail.
2. Read the first useful failure.
3. Compare the branch with `main`.
4. Explain what the result proves and what it does not prove.
